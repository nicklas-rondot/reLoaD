...
void setup() {
  ...
  motor.PID_velocity.P = 10.0f; // Had it on 6 before for velocity tests but then at low speeds strong rumbling
  ...
  motor.P_angle.output_ramp = 900;  // 900 no load
  motor.P_angle.P = 15;             //27.5 no load. Had it on 180 for position tests but may lead to rumbing. must fit to lpf filter 
  motor.P_angle.I = 0;
  motor.velocity_limit = 10;  // [rad/s]

  //motor.voltage_limit = 7;
  motor.current_limit = 7; // 7 means less than 1.5A => 2200RPM with load, scale seems off. 5 is less than 1A
  ...
  motor.zero_electric_angle  = 2.54 + initial_offset; // rad
